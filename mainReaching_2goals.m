...
rx1 = -40;        % Initial x-position of the target 1
ry1 = 40;        % Initial y-position of the target 1

rx2 = 40;         % Initial x-position of the target 2
ry2 = 40;         % Initial y-position of the target 2

%Goal-related decision variables 
R1 = 1;           % Reward for target 1
R2 = 1;           % Reward for target 2

Prior_1 = 1;      % Prior probability of target 1 
Prior_2 = 1;      % Prior probability of target 2
...
neuralParam.q_u = .1; % noise levels
...
dnf.history_output = zeros(nTrials * length(tStoreFields), neuralParam.fieldSize);
...
for i = 1 : nTrials
  ...
  for t = 1 : tMax
    ...
    stim1 = 3*gauss(1:neuralParam.fieldSize, round(theta1), 5); % a localized input
    stim2 = 3*gauss(1:neuralParam.fieldSize, round(theta2), 5); % a localized input
    bias_stim1 = 0.5*gauss(1:neuralParam.fieldSize, round(theta1), 5); % a localized input
    bias_stim2 = 0.5*gauss(1:neuralParam.fieldSize, round(theta2), 5); % a localized input

    stimulus = bias_stim1+bias_stim2;
    if t>= tStimulusStart && t<tStimulusEnd
        stimulus = stimulus+stim1+stim2;
    end
    rel_desir1=3*RelCost_T1*gauss(1:neuralParam.fieldSize, round(theta1), 5); % a localized input
    rel_desir2=3*RelCost_T2*gauss(1:neuralParam.fieldSize, round(theta2), 5); % a localized input
    stimulus = stimulus+rel_desir1+rel_desir2;
    ...
    dnf.field_u = dnf.field_u + 1/neuralParam.tau_u * (-dnf.field_u + neuralParam.h_u + stimulus + ...
    ...
    if any(dnf.activeRegions_u)
      ...
      dnf.memTrace_u = dnf.memTrace_u + 1/neuralParam.tau_mu_build * (-dnf.memTrace_u + dnf.output_u) + ...
          1/neuralParam.tau_mu_decay * (-dnf.memTrace_u);
    end
    ...
    if t==tStimulusStart
        ...
        [Cost_commands_T1 Cost_accuracy_T1 Overall_action_cost_T1] = ActionCost(Unoise_T1,Q,R,Xhat_T1);
        ...
        [Cost_commands_T2 Cost_accuracy_T2 Overall_action_cost_T2] = ActionCost(Unoise_T2,Q,R,Xhat_T2);
    
        %relative desirability of the two policies (combine action cost, with
        %reward and prior probability values)
        RelCost_T1 = exp(-Overall_action_cost_T1/lambda)/(exp(-Overall_action_cost_T1/lambda) + exp(-Overall_action_cost_T2/lambda))
        RelCost_T2 = exp(-Overall_action_cost_T2/lambda)/(exp(-Overall_action_cost_T1/lambda) + exp(-Overall_action_cost_T2/lambda))
    end
    if max(dnf.output_u)>move_threshold
        disp('we are start moving')
        % start moving
%         weight_T1 = mean(
%         Uexecuted = mean(dnf.output_u)*
